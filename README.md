...
installed: `muzaiten-features-clap` from the AUR, or the published Python
tool on any distribution. The app never runs a package manager or downloads
a model without consent:
...
After a compiler, Qt, or packaged dependency upgrade, `make build` recompiles
the build directory from scratch on its own; driving CMake directly needs
