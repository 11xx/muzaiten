...
- **Stack:** Qt 6 Widgets / **C++26** / CMake+Ninja, wrapped by a `Makefile`. `make build` = incremental build; `make test` = run the suite; `make dev` = build + run against an isolated `./dev-state`; `make rebuild` = clean build (only for CMake/cache changes or stale-build symptoms). A compiler, Qt, or packaged dependency upgrade is handled by `make build` itself: it records the toolchain each configure and recompiles from scratch when it changes.
