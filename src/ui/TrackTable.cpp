...
enum TrackRoles {
    ...
    TrackIdentityRole = TableViewState::IdentityRole,
};
...
public:
    ...
    QVariant data(const QModelIndex &index, int role) const override
    {
        ...
        if (role == TrackIdentityRole) {
            return track.path.isEmpty() ? QVariant() : track.path;
        }
        if (role == TrackRole) {
...
void TrackTable::setTracks(const QVector<Track> &tracks)
{
    ...
    const TableViewState::Snapshot state = TableViewState::capture(*this);
    ...
    TableViewState::restore(*this, state);
    reselectMarkedRows();
}
